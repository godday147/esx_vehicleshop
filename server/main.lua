...
Citizen.CreateThread(function()
	...
	if char > 8 then
		print(('[車輛經銷商]: ^1警告^7 車牌號碼字母已超過 %s/8 字符.'):format(char))
	end
end)
...
AddEventHandler('esx_vehicleshop:addToList', function(target, model, plate)
	...
	if xPlayer.job.name ~= 'cardealer' then
		print(('[車輛經銷商]: %s 試圖將已售出的車輛添加到列表中!'):format(xPlayer.identifier))
		return
	end
...
ESX.RegisterServerCallback('esx_vehicleshop:buyVehicle', function (source, cb, vehicleModel)
	...
	if xPlayer.getMoney() >= vehicleData.price then
		...
		TriggerEvent("nuo:buycar",xPlayer.name, vehicleData.name, vehicleData.price)
		cb(true)
	else
		cb(false)
	end
...
AddEventHandler('esx_vehicleshop:returnProvider', function(vehicleModel)
	...
	MySQL.Async.fetchAll('SELECT * FROM cardealer_vehicles WHERE vehicle = @vehicle LIMIT 1', {
		...
		if result[1] then
			...
			print(('[車輛經銷商]: %s ^1試圖出售無效車輛 !!^7'):format(GetPlayerIdentifiers(_source)[1]))
		end
...
ESX.RegisterServerCallback('esx_vehicleshop:resellVehicle', function (source, cb, plate, model)
	...
	if resellPrice == 0 then
		print(('[車輛經銷商]: %s ^1試圖出售一輛未知車輛!^7'):format(GetPlayerIdentifiers(source)[1]))
		cb(false)
	else
		MySQL.Async.fetchAll('SELECT * FROM rented_vehicles WHERE plate = @plate', {
			['@plate'] = plate
					...
					if result[1] then -- does the owner match?
						...
						if vehicle.model == model then
							if vehicle.plate == plate then
								...
								RemoveOwnedVehicle(plate)
								TriggerEvent("nuo:sellcar",xPlayer.name, vehicle.plate, resellPrice,vehicleData.name)
								cb(true)
							else
								print(('[車輛經銷商]: %s 試圖出售車牌不匹配的車輛!'):format(xPlayer.identifier))
								cb(false)
							end
						else
							print(('[車輛經銷商]: %s 試圖出售模型不匹配的車輛!'):format(xPlayer.identifier))
							cb(false)
						end
					else
							...
							MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND @plate = plate', {
								...
								if result[1] then
									...
									if vehicle.model == model then
										...
										print(('[車輛經銷商]: %s 試圖出售模組不匹配的車輛!'):format(xPlayer.identifier))
										cb(false)
									end
								else
									cb(false)
...
AddEventHandler('esx_vehicleshop:setJobVehicleState', function(plate, state)
	...
	MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = @stored WHERE plate = @plate AND job = @job', {
		...
		if rowsChanged == 0 then
			print(('[車輛經銷商]: %s 將車輛停入車庫中!'):format(xPlayer.identifier))
		end
	end)
end)
